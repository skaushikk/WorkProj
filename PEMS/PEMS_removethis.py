...
def PEMS(Fileinfo, Tloads, PipeEndA, WinchB, Contents, Tol):
    ...
    pipeline.EndAxBendingStiffness =  Tloads[2]
...
def PEMS_Analysis(filepathinfo):
    ...
    TEN_ten_E = np.array(sh1.range('H15').options(expand='right').value)
    TEN_ten_F = np.array(sh1.range('H16').options(expand='right').value)
    HOM_ten_E = np.array(sh1.range('H17').options(expand='right').value)
    HOM_ten_F = np.array(sh1.range('H18').options(expand='right').value)
    ...
    for twn in range(np.size(TowerAngles)):
        ...
        for htn in ht:
            for cn in con:
                ...
                tenmax = eval(htn + '_ten_' + cn[0]+ '[0]')
                tens = np.linspace(35, tenmax, num=4)

                ht_stiff = eval(htn + '_ten_' + cn[0]+ '[1]')
                ...
                while k < 5:
                    ...
                    Tloads = [TargetTen, TargetBM, ht_stiff]
                    print('\n\n',twa, htn, cn)
