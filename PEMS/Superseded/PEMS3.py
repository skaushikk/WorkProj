...
def createCSV(LCFolder):
    wb = xw.Book.caller()
    sh1 = wb.sheets('PEMS')
    shtemp = wb.sheets('temp')
    
    rowsno = 137
    writefile =open(LCFolder +'.csvt', 'w', newline='')
...
def batchfileappend(LCFolder):
    batchfile = open('csv1.bat', 'a')
    batchfile.write(LCFolder +'.csvt ')
    batchfile.write('+')
...
#    wb = xw.Book.caller()
    ...
    BatchRunData = np.asarray([TowerAngles, HOMorTEN, EMPoFLD])
    ...
    #                pipeEndAZ = htrl[np.where(HOM_Reel_pt[:,0] == twa)][0,2]
                    ...
                    while k<5:
                        ...
                        FNameID = pipe_ID + '_' + str(twa) + 'deg_' + htn + '_' + cn + '_' + str(np.int(tension)) + 'kN_' +   str(np.int(bendingmoment)) + 'BM.dat'
    ...
    #                    RangeL = RangeX.X
                        ...
                        if k > 0:
                            ...
                            RangeDeflection = np.sqrt((RangeXstack[k]-RangeXstack[0])**2+(RangeZstack[k]-RangeZstack[0])**2)
        ...
        #            shtemp.activate()
        #            df = pd.read_excel(shtemp)
                    createCSV(LCFolder)
                    batchfileappend(LCFolder)
    ...
    batchfile.write('del /s /q /f *.csvt')
    batchfile.close()
...
if __name__ == "__main__":
    xw.Book('PEMS2.xlsm').set_mock_caller()
    PEMS_Analysis ()
