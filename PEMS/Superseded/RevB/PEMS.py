...
def PEMS(FPath, FName, TTen, TBM, EndA, WinchB, Content):
   ...
   # logic loop
    ...
    while tension_error > 0.1 or bending_moment_error > 0.01:
            ...
            if loop_count > 5000:
                break
            print("loop number ",loop_count,"  Ten= ",round(pipeline_tension,1),"  BM= ",round(pipeline_bending_moment,1),"WinchX=",pipelineWinchX)
...
if __name__ == "__main__":
    ...
    i=1
    while i<6:       
        FNameIndex = ws.cell(row=38+i,column=1).value
        FName1= str(filePath) + "\\" + str(LCFolder) + "\\"+ str(FNameIndex)
