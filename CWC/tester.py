...
LtoFJ = np.arange(4000,4200,200)        #Distance of the section that moment is calculated from the field joint
yNA = np.zeros((len(LtoFJ),len(Eg)))
Mplot = yNA*0
Kplot = yNA*0
Fsteelplot =yNA*0
Faclplot = yNA*0
...
for l in range(len(LtoFJ)):
    lFJ = LtoFJ[l]

    Fshc_max = (np.pi)*OD*lFJ*Ssh_acl/(10**(6)) #The shear force that stops concrete fom sliding
    ...
    for k in range(len(Eg)):
        ...
        ##         ## Stress Calculation for steel, acl and concrete
             for i in range(0,2*Ntot):
                 ...
                 Sax[i][2] = np.sign(e[i])*findstressSteel3(abs(e[i]),SteelProp)
...
#                     print(slide)
    ...
    #         print(Sax[1,0])
        ...
        Kplot[l,k] = kappag
        yNA[l,k] = yTot[0]-yi[0]
        Fsteelplot[l,k]=Fsum[2]
        Faclplot[l,k]=Fsum[1]
        Fconplot[l,k]=Fsum[0]
#        print(k,count,er,dl)
        print('l=',l,'k=',k, 'yNA=',yNA[l,k])
